...
const PAIRING_SERVICE_URL = process.env.PAIRING_SERVICE_URL || `http://127.0.0.1:${process.env.PORT || 8000}`;

cmd({
    pattern: "pair",
    alias: ["getpaijsksnsr", "pairing", "clonebnsjdndnznot"],
    react: "✅",
    desc: "Get pairing code for DTZ VISHI MD bot",
    category: "download",
    use: ".pair 92323***",
    filename: __filename
    ...
    try {
        ...
        const response = await axios.get(`${PAIRING_SERVICE_URL}/code?number=${encodeURIComponent(phoneNumber)}`);
