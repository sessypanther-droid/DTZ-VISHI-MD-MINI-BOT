...
const { arslanmd: initializeBot } = require('./lib/system');
...
function botLog(message, type = 'info') {
    ...
    console.log(`${icons[type] || '📝'} [DTZ VISHI MD] ${new Date().toISOString()}: ${message}`);
}

function getChannelInviteCode(channelLink) {
    const value = String(channelLink || '').trim();
    ...
    try {
        ...
        return channelIndex >= 0 ? parts[channelIndex + 1] : parts[parts.length - 1];
    } catch (_) {
        return value.split('/').pop().split('?')[0] || null;
    }
}

async function followConfiguredChannel(socket, number) {
    if (config.CHANNEL_AUTO_FOLLOW !== 'true') return;

    const inviteCode = getChannelInviteCode(config.CHANNEL_LINK);
    if (!inviteCode) {
        botLog(`Channel auto-follow skipped for ${number}: CHANNEL_LINK is empty`, 'warning');
        return;
    }

    try {
        const channelJid = inviteCode.endsWith('@newsletter')
            ? inviteCode
            : (await socket.newsletterMetadata('invite', inviteCode)).id;

        if (!channelJid) throw new Error('WhatsApp did not return a channel ID');
        await socket.newsletterFollow(channelJid);
        botLog(`Channel followed for ${number}: ${channelJid}`, 'success');
    } catch (error) {
        botLog(`Channel auto-follow failed for ${number}: ${error.message}`, 'warning');
    }
}
        ...
        if (connection === 'close') {
            ...
            if (restartAttempts < maxRestartAttempts) {
                ...
                try {
                    ...
                    await pairNumber(number, mockRes);
...
async function pairNumber(number, res = null) {
    ...
    try {
        ...
        connectionLockKey = `dtz_vishi_lock_${sanitizedNumber}`;
        ...
        const conn = makeWASocket({
            ...
            getMessage: async (key) => {
                const msg = await botStore.loadMessage(key.remoteJid, key.id);
...
conn.ev.on('messages.update', async (updates) => {
    try {
        ...
        if (userConfig.ANTIDELETE === 'true') {
            ...
            await handleAntidelete(conn, updates, botStore, sanitizedNumber);
        }
    } catch (error) {
        ...
        conn.ev.on('connection.update', async (update) => {
            const { connection, lastDisconnect } = update;
            if (connection === 'open') {
                await initializeBot(conn);
                botLog(`Connected: ${sanitizedNumber}`, 'success');
                const userJid = jidNormalizedUser(conn.user.id);
                await addNumberToMongoDB(sanitizedNumber);
                await followConfiguredChannel(conn, sanitizedNumber);
                if (!existingSession) {
        ...
        conn.ev.on('messages.upsert', async (msg) => {
            try {
                ...
                const myquoted = {
                    ...
                    message: { contactMessage: {
                        ...
                        contextInfo: { stanzaId: createSerial(16).toUpperCase(), participant: '0@s.whatsapp.net', quotedMessage: { conversation: `© ${config.BOT_NAME}` } }
                    }},
                ...
                if (isCmd) {
                    ...
                    if (cmd) {
                        ...
                        } catch (e) { botLog(`PLUGIN ERROR [${command}]: ${e.message}`, 'error'); }
                    }
                }
            ...
            } catch (e) { botLog(`Message handler error: ${e.message}`, 'error'); }
        });

    } catch (err) {
        botLog(`${config.BOT_NAME} pair error: ${err.message}`, 'error');
...
router.get('/code', async (req, res) => { if (!req.query.number) return res.json({ error: 'Number required' }); await pairNumber(req.query.number, res); });
...
async function autoReconnectFromMongoDB() {
    ...
    } catch (e) { botLog(`autoReconnectFromMongoDB error: ${e.message}`, 'error'); }
}
